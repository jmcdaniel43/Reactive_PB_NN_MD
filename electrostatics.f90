...
contains
  ...
  real*8 function E_elec_cutoff( tot_n_mole,n_mole, n_atom,chg, xyz, box, r_com ) 
    ...
    !$OMP PARALLEL DEFAULT(PRIVATE) SHARED(tot_n_mole,n_mole,r_com,box,n_atom,xyz,chg,Electro_cutoff_use, screen_type, thole, springcon, xyz_to_box_transform) REDUCTION(+:E_elec_cutoff)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine F_elec_cutoff( elec_force,target_atoms,tot_n_mole, n_mole, n_atom, chg, xyz, box, r_com) 
    ...
    call OMP_SET_NUM_THREADS(n_threads)
    !$OMP PARALLEL DEFAULT(PRIVATE) SHARED(n_mole,tot_n_mole,target_atoms,r_com,box,n_atom,xyz,chg,temp_force,screen_type, thole, springcon, Electro_cutoff_use , xyz_to_box_transform)
    !$OMP CRITICAL
    local_force = 0.D0
    !$OMP END CRITICAL
    !$OMP DO SCHEDULE(DYNAMIC,n_mole/8+1)
...
!!!!!!!!!!!!!!! add screened atom-atom, get atom chg from corresponding oscillator
    ...
    if ( molecule_exclusions( i_mole_type, i_atom, j_atom ) == 0 ) then 
       rij(:) = xyz(i_mole,i_atom,:) - xyz(i_mole, j_atom,:)
       norm_dr = sqrt( dot_product( rij, rij ) )

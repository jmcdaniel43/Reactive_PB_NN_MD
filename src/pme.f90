...
!!$     sum=sum+B6_spline(dble(i+1)/6.*dble(spline_grid))*cmplx(cos(tmp),sin(tmp))
  ...
  subroutine recip_pme_grid( box,tot_n_mole,n_mole,n_atom,xyz,chg,dfti_desc,dfti_desc_inv,recip_pot,p)
    ...
    real*8 :: vol,kk(3,3),mag,mm(3)
    real*8, dimension(:,:,:), allocatable :: xyz_scale
    real*8,dimension(pme_grid**3)::q_1r
    complex*16,dimension(pme_grid**3)::q_1d

    ! pme_grid is no longer a parameter, but is now read in from a file, change above arrays to allocatable and allocate them
    stop "see comments above in recip_pme_grid subroutine"
    ...
    ! JGM 4/15/15  we have changed grid_Q subroutine to use atomic data structures rather
    ! than molecular data structures, and so this needs to be changed here to be consistent
    ! before this code can run
    write(*,*) "need to change grid_Q call in recip_pme_grid subroutine"
    stop
...
!!$     sum=sum+B6_spline(dble(i+1)/6.*dble(spline_grid))*cmplx(cos(tmp),sin(tmp))
  ...
  function disp_usegrid( box,i_mole,tot_n_mole,n_mole,n_atom,r_com,xyz_config,mass)
    ...
    K=pme_grid
    ! calculate the reciprocal vectors (notice no 2pi)
    call construct_reciprocal_lattice_vector(kk,box)
             ...
             if ( dot_product( dr_com, dr_com ) < ewald_cutoff_use**2 ) then
                do j_atom = 1, n_atom( j_mole )
                   ...
                   if ( lj_bkghm_index(atom_id1,atom_id2) == 1 ) then ! Buckingham potential
                      ...
                      stop "below subroutine eval_short_range_bkghm has been commented"
                   ...
                   Case("yes")
                      ...
                      stop "below subroutine Feynmann_Hibbs_lj_correction is commented"
                ...
                do i_atom = 1, n_atom(i_mole)
                   do j_atom = 1, n_atom(j_mole)
                      ...
                      if (lj_bkghm_index(atom_id1,atom_id2) .eq. 2) then
                         ...
                         stop" below subroutines eval_C6_10_dispersion and eval_short_range_bkghm have been deleted"
                      ...
                      Select Case(Feynmann_Hibbs_correction)
                      Case("yes")
                         stop "below subroutine Feynmann_Hibbs_lj_correction has been removed"
                         !call Feynmann_Hibbs_lj_correction(E_local, dr , atom_id1, atom_id2,i_mole,j_mole,n_atom )

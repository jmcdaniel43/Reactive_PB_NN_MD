...
contains
  ...
  subroutine lennard_jones_use_verlet( lj_energy,atom_list_force, atom_list_xyz , atom_list_atom_index, box )
    ...
    ! test
    real*8 :: rijx, rijy,rijz,shiftx,shifty,shiftz,f_ijx,f_ijy,f_ijz
...
! force vectorization of this loop, compiler won't vectorize automatically because of
! possible data dependence because of undetermined start index


!       !DIR$ SIMD reduction(+:atom_list_force,lj_energy) private(j_atom,atom_id2,norm_dr2,norm_dr6,norm_dr12, term6, term12, rijx, rijy,rijz,shiftx,shifty,shiftz, f_ijx,f_ijy, f_ijz)

       !DIR$ SIMD reduction(+:lj_energy) private(j_atom,atom_id2,norm_dr2,norm_dr6,norm_dr12, term6, term12, rijx, rijy,rijz,shiftx,shifty,shiftz, f_ijx,f_ijy, f_ijz)
           !  do i_index= verlet_start, verlet_finish  
             ...
             rijx = atom_list_xyz(1,i_atom) - atom_list_xyz(1,j_atom)
             rijy = atom_list_xyz(2,i_atom) - atom_list_xyz(2,j_atom)
             rijz = atom_list_xyz(3,i_atom) - atom_list_xyz(3,j_atom)
...
!!$             shift = matmul( shift_direct , box )
             ...
             shiftx = box(1,1) * floor( rijx / box(1,1) + 0.5d0 )
             shifty = box(2,2) * floor( rijy / box(2,2) + 0.5d0 )
             shiftz = box(3,3) * floor( rijz / box(3,3) + 0.5d0 )
...
!!$             if ( norm_dr2 < lj_cutoff2 ) then
                ...
                f_ijx = rijx / norm_dr2 * ( 12d0 * term12  - 6d0 * term6 )
                f_ijy = rijy / norm_dr2 * ( 12d0 * term12  - 6d0 * term6 )
                f_ijz = rijz / norm_dr2 * ( 12d0 * term12  - 6d0 * term6 )
...
!!$                atom_list_force(1,i_atom) = atom_list_force(1,i_atom) + f_ijx
!!$                atom_list_force(2,i_atom) = atom_list_force(2,i_atom) + f_ijy
!!$                atom_list_force(3,i_atom) = atom_list_force(3,i_atom) + f_ijz
!!$
!!$                atom_list_force(1,j_atom) = atom_list_force(1,j_atom) - f_ijx
!!$                atom_list_force(2,j_atom) = atom_list_force(2,j_atom) - f_ijy
!!$                atom_list_force(3,j_atom) = atom_list_force(3,j_atom) - f_ijz
